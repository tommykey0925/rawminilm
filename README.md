...
| 表 | 大きさ | 何をするための表か |
|----|--------|----|
| 表1 | 9文字 × 8個の数値 = 72個 | 文字番号を渡すと、その文字の「特徴」を8個の数値で返してくれる表。辞書みたいなもの |
| 表2 | 8 × 9 = 72個 | 表1から出てきた8個の数値を使って、次にどの文字が来そうかのスコアを計算するための表 |
| 表3 | 9個 | スコアの微調整に足す数値。表2だけだと調整しきれない部分を補う |
...
flowchart TD
    A["入力: 文字番号<br>例: 4 ('も')"]
    B["表1を引く<br>'も' の特徴を8個の数値で取り出す"]
    C["表2・表3で計算<br>8個の数値からスコアを出す<br>(かけ算と足し算だけ)"]
    D["9文字ぶんのスコア<br>例: た=3.1, も=2.8, う=0.5, ..."]
...
**表1を引く**
文字番号を使って表1からその文字の行を取り出す。「も」= 番号4 なら、表1の4行目を取り出す。8個の数値が出てくる。最初は適当な数値だけど、学習を繰り返すうちに意味のある値になっていく。
...
```mermaid
flowchart TD
    A["スコアへの修正指示を求める<br>予測確率 - 正解<br>例: も → 0.45-1=-0.55 (もっと上げろ)<br>例: れ → 0.08-0=+0.08 (下げろ)"]
    B["表2・表3への修正指示を求める"]
    C["表1への修正指示を求める<br>(修正指示を逆向きにたどっていく)"]
