...
# だから最初に、文字に番号を振る。
# 例: う=0, か=1, た=2, は=3, も=4, ら=5, ろ=6, う=7, れ=8
...
class TinyLM:
    def __init__(self, vocab_size: int, emb_dim: int = 8) -> None:
        ...
        # 学習を繰り返すうちに、似た使われ方をする文字は似た数値になっていく。
        # この仕組みを「Embedding (埋め込み)」と呼ぶ。
        ...
        # この計算を「Linear (線形変換)」と呼ぶ。中身はかけ算と足し算だけ。
    ...
    # =============================================
    # ② 数値の表を使って「次の文字」を予測する (Forward)
    # =============================================
    # 「Forward (順伝播)」= データを前に流して予測を出す処理。
    ...
    #   正解が「た」で、「た」に80%を振れていたら → まあまあ良い → lossは小さい
    #   正解が「た」で、「た」に2%しか振れてなかったら → ひどい → lossは大きい
    #
    ...
    # この仕組みを「Cross Entropy Loss (交差エントロピー損失)」と呼ぶ。
    ...
    # =============================================
    # ④ ハズレを減らす方向に表の数値を直す (Backward + SGD)
    # =============================================
    # lossを測ったら、次は「表のどの数値を、どっちの方向に、どれだけ直せばいいか」を計算する。
    # この「修正指示」のことを「勾配 (gradient)」と呼ぶ。
    #
    # イメージ: 山の中で目隠しされている。一番低い谷に行きたい(= lossを最小にしたい)。
    # 足元の傾きを感じて「こっちが下り坂だ」とわかったら、そっちに一歩進む。
    # また傾きを感じて、また一歩。これを繰り返して谷を目指す。
    # 「足元の傾き」が勾配で、「一歩進む」が表の数値を直す作業。
    #
    # そして、勾配を使って実際に表の数値を直すやり方を
    # 「SGD (確率的勾配降下法)」と呼ぶ。やってることは単純で:
    #   新しい数値 = 今の数値 - 学習率 × 勾配
    ...
    def backward(self, probs: NDArrayFloat, Y: NDArrayInt, h: NDArrayFloat, X: NDArrayInt, lr: float) -> None:
        ...
        # --- まず、スコア(logits)に対する修正指示を求める ---
        # ここの計算は、softmaxとcross entropyをまとめて微分すると
        # 「予測確率 - 正解」というきれいな形になる(数学的に導けるが、結果だけ使えばOK)。
        ...
        # これが「逆伝播 (Backpropagation)」。後ろから前に修正指示を伝えていく仕組み。
