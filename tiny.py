...
class TinyLM:
    def __init__(self, vocab_size: int, emb_dim: int = 8) -> None:
        ...
        self.emb_weight: NDArrayFloat = np.random.randn(vocab_size, emb_dim)          # Embedding: (9, 8)
    ...
    def forward(self, X: NDArrayInt) -> tuple[NDArrayFloat, NDArrayFloat]:
        ...
        logits_shifted: NDArrayFloat = logits - max_val  # オーバーフロー防止
    ...
    def backward(self, probs: NDArrayFloat, Y: NDArrayInt, h: NDArrayFloat, X: NDArrayInt, lr: float) -> None:
        ...
        dlinear_weight: NDArrayFloat = np.dot(h.T, dlogits)  # (8, 9)

        # Linear のバイアスの勾配: db = Σ_i dlogits[i,j]
        # → 修正指示を全サンプル分足し合わせたもの
        dlinear_bias: NDArrayFloat = np.sum(dlogits, axis=0)  # (9,)
    ...
    def generate(self, start_char: str, length: int, vocab: Vocab) -> str:
        ...
        for _ in range(length):
            ...
            h: NDArrayFloat = self.emb_weight[x_idx]  # (8,)

            # Linear: dot(h, W) + b
            logits: NDArrayFloat = np.dot(h, self.linear_weight) + self.linear_bias  # (9,)

            # softmax でスコアを確率に変換 (学習時と同じ計算)
            logits_shifted: NDArrayFloat = logits - np.max(logits)  # 最大値を引く (数値安定性のため)

            exp_logits: NDArrayFloat = np.exp(logits_shifted)  # e^x を計算
...
X: NDArrayInt = vocab.encode(text[:-1])  # 入力: (13,)
Y: NDArrayInt = vocab.encode(text[1:])   # 正解: (13,)
