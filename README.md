...
確率からハズレの度合いを出せる数学の関数 (log) を使う。log は確率を入れると 0 かマイナスの値を返す関数で、確率が低いほど大きなマイナスになる:
...
ハズレの度合いは「大きいほど悪い」にしたいのに、log の結果はマイナスになる。だからマイナスをつけて符号をひっくり返す（コードでは `-np.log(確率)` の部分）。すると:

```
-log(0.8)  = 0.22   (ハズレ小さい)
-log(0.1)  = 2.30   (ハズレ大きい)
-log(0.02) = 3.91   (ハズレひどい)
```

なぜ log が向いているかというと、確率が低くなるほどハズレの度合いが急激に大きくなる性質があるから。80%→60%で増えるハズレより、10%→2%で増えるハズレのほうがずっと大きい。だから「ひどい予測」ほど優先的に直す効果がある。
