...
3つの表すべてに修正指示が行き渡ったら、実際に数値を直す。

- **学習率** (`lr`): 一度にどれだけ大きく直すかの設定値。プログラムの実行部分で自分で決める（このコードでは `lr = 1.0`）。大きくすればたくさん直す、小さくすれば少しずつ直す
- **修正指示** (`demb_weight` 等): 上の「予測確率 - 正解」から順番に計算してきた結果。「この数値は〇〇ぶんだけ直したほうがいい」という指示
...
例: 「も」の変換表の3番目の数値が今 `0.30` で、修正指示が `0.05` (下げろ) なら:
```
0.30 - 1.0 × 0.05 = 0.25   → 0.30 が 0.25 に直された
```
これを変換表・点数表・調整値の全数値に対してやる。
