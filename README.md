...
ここが学習の本体。`loss()` の値を直接使うわけではなく、予測確率と正解から「変換表・点数表・調整値のどの数値を、どっちの方向に、どれだけ直せばいいか」の修正指示を計算して、実際に数値を直す。
...
修正指示はこの逆の順番で、パーセント側から変換表に向かって求めていく:
```
修正指示（逆向き）:
  パーセントと正解の差 → 点数表への修正指示 → 調整値への修正指示 → 変換表への修正指示
```
...
flowchart LR
    subgraph 予測["予測（順向き）"]
        direction LR
        A1["変換表"] --> A2["点数表"] --> A3["調整値"] --> A4["点数"] --> A5["パーセント"]
    end

    subgraph 修正["修正指示（逆向き）"]
        direction RL
        B1["パーセントと<br>正解の差"] --> B2["点数表への<br>修正指示"] --> B3["調整値への<br>修正指示"] --> B4["変換表への<br>修正指示"]
    end
```
...
**① パーセントと正解の差を出す**
出発点はここ。「予測確率 - 正解」で、点数に対する修正指示を求める。
- 「も」が正解で、予測が「も=45%, た=20%, ...」だった場合:
  - 「も」: 0.45 - 1 = -0.55 →「あと55%ぶん上げろ」
  - 「た」: 0.20 - 0 = +0.20 →「20%ぶん下げろ」

**② 点数表への修正指示を求める**
①の結果を使って、点数表の中のどの数値をどれだけ直すかを計算する。

**③ 調整値への修正指示を求める**
①の結果を使って、調整値のどの数値をどれだけ直すかを計算する。

**④ 変換表への修正指示を求める**
①の結果と点数表の値を使って、変換表のどの数値をどれだけ直すかを計算する。予測では変換表が一番最初だったが、修正指示では一番最後に求まる。
...
- **学習率** (`lr`): 一度にどれだけ大きく直すかの設定値。プログラムの実行部分で自分で決める（このコードでは `lr = 1.0`）。大きくすればたくさん直す、小さくすれば少しずつ直す
- **修正指示**: 上の①〜④で求めた各表への修正指示

```
変換表:  変換表の値  - lr × 変換表への修正指示  (④で求めたもの)
点数表:  点数表の値  - lr × 点数表への修正指示  (②で求めたもの)
調整値:  調整値の値  - lr × 調整値への修正指示  (③で求めたもの)
```

例: 変換表の中のある数値が今 `0.30` で、④で求めた修正指示が `0.05` (下げろ) なら:
```
0.30 - 1.0 × 0.05 = 0.25   → 0.30 が 0.25 に直された
```
...
2. この確率に従って1文字を選ぶ。60%の文字は60%の確率で、2%の文字は2%の確率で選ばれる → 「も」が選ばれた
3. 選ばれた「も」を次の入力にする → また予測が出る → また1文字選ぶ → 「た」が選ばれた
4. 「た」を入力 → ... を繰り返して「ももたろうは...」のような文章ができあがる
