...
flowchart LR
    A["入力: 文字番号<br>例: 4 ('も')"]
    B["表1を引く<br>(Embedding)<br>8個の数値を取り出す"]
    C["表2・表3で計算<br>(Linear)<br>かけ算と足し算"]
    D["9文字ぶんのスコア<br>例: た=3.1, も=2.8, ..."]
    E["パーセントに変換<br>(softmax)<br>足して100%になるように"]
...
予測が出たら、次は「どのくらい間違えたか」を1つの数値にしたい。この数値を「**loss (損失)**」と呼ぶ。大きいほどハズレがひどく、小さいほど良い予測。

考え方はシンプルで、正解の文字に何%を振れたかを見る:
- 正解が「た」で、「た」に80%を振れていた → 良い → loss は小さい
- 正解が「た」で、「た」に2%しか振れてなかった → ひどい → loss は大きい
...
なぜ log を使うかというと、確率が低いときにペナルティがドカンと跳ね上がるから。50%→10% の悪化より、10%→1% の悪化のほうが深刻で、log はそれを自然に反映してくれる。この仕組みを「**Cross Entropy Loss**」と呼ぶ。

### ④ ハズレを減らす方向に表の数値を直す — `backward()`

loss を測ったら、次は「表のどの数値を、どっちの方向に、どれだけ直せばいいか」を計算する。この「修正指示」のことを「**勾配 (gradient)**」と呼ぶ。

イメージ: 山の中で目隠しされている状態で、一番低い谷に行きたい（= loss を最小にしたい）。足元の傾きを感じて「こっちが下り坂だ」とわかったら、そっちに一歩進む。また傾きを感じて、また一歩。これを繰り返して谷を目指す。「足元の傾き」が勾配で、「一歩進む」が表の数値を直す作業。
...
flowchart RL
    ...
    B["表2・表3への修正指示"]
    C["表1への修正指示"]
    D["実際に表の数値を直す<br>(SGD)<br>新しい値 = 今の値 - 学習率 × 勾配"]

    A --> B --> D
    A --> C --> D
```

修正指示を後ろから前に伝えていく仕組みを「**逆伝播 (Backpropagation)**」と呼ぶ。

勾配を使って実際に表の数値を直すやり方を「**SGD (確率的勾配降下法)**」と呼ぶ:
```
新しい数値 = 今の数値 - 学習率 × 勾配
```
...
| 名前 | 形 | 役割 |
|------|------|------|
| 表1: `emb_weight` (Embedding) | 9×8 = 72個 | 文字番号 → 8個の特徴数値 |
| 表2: `linear_weight` (Linear) | 8×9 = 72個 | 特徴数値 → 各文字のスコア |
| 表3: `linear_bias` (Linear) | 9個 | スコアに足す調整値 |
| **合計** | **153個** | |
