...
class Vocab:
    def __init__(self, text):
        ...
        self.size = len(self.chars)  # 文字の種類数
        self.to_i = {c: i for i, c in enumerate(self.chars)}
        self.to_c = {i: c for i, c in enumerate(self.chars)}
...
class TinyLM:
    def __init__(self, vocab_size, emb_dim=8):
        ...
        self.emb_weight = np.random.randn(vocab_size, emb_dim)          # Embedding: (9, 8)
        ...
        self.linear_weight = np.random.uniform(-bound, bound, (emb_dim, vocab_size))  # (8, 9)
        # np.zeros(n): 長さ n のゼロベクトルを作る → 全部ゼロで初期化 (最初は偏りなしでスタート)
        self.linear_bias = np.zeros(vocab_size)                                        # (9,)
    ...
    def backward(self, probs, Y, h, X, lr):
    ...
    def generate(self, start_char, length, vocab):
        """生成: 1文字ずつ予測を繰り返す"""
        ch = start_char
        result = ch
        for _ in range(length):
            ...
            h = self.emb_weight[x_idx]  # (8,)

            # Linear: dot(h, W) + b
            logits = np.dot(h, self.linear_weight) + self.linear_bias  # (9,)
...
X = vocab.encode(text[:-1])  # 入力: (13,)
Y = vocab.encode(text[1:])   # 正解: (13,)

model = TinyLM(vocab.size)
