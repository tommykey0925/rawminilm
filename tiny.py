...
class TinyLM:
    ...
    # この数値は学習の進み具合を確認するためのもので、なくても学習自体は動く
    # (実際に表を直すのは次の backward() の仕事)。
    #
    # 考え方はシンプル:
    ...
    # 学習②: 3つの表の数値を直す
    # ==========================================================================
    # ここが学習の本体。loss() の値を直接使うわけではなく、
    # 予測確率と正解から「変換表・点数表・調整値への修正指示」を計算して数値を直す。
    #
    # 修正指示は予測と逆の順番で求めていく:
    #   予測:     文字 → [変換表] → [点数表]+[調整値] → 点数 → パーセント
    #   修正指示: パーセントと正解の差 → 点数表 → 調整値 → 変換表
    #
    # 修正指示が出たら、3つの表すべてに同じ式で数値を直す:
    #   新しい数値 = 今の数値 - 学習率(lr) × 修正指示
    #   lr = 一度にどれだけ大きく直すかの設定値 (このコードでは 1.0)
    ...
    # あとは予測を繰り返すだけ:
    #   1. 「も」を入力 → 予測が出る (も=60%, た=20%, ...)
    #   2. 確率に従って1文字選ぶ → 「も」が選ばれた
    #   3. 「も」を次の入力にする → また予測 → 「た」が選ばれた
    ...
    def generate(self, start_char: str, length: int, vocab: Vocab) -> str:
        ...
        for _ in range(length):
            ...
            # 予測された確率に従って1文字を選ぶ。
            # 60%の文字は60%の確率で、2%の文字は2%の確率で選ばれる。
